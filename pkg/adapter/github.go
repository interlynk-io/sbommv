...
// FetchSBOMs initializes the appropriate GitHub SBOM iterator based on the method
func (g *GitHubAdapter) FetchSBOMs(ctx context.Context) (iterator.SBOMIterator, error) {
	client := github.NewClient(g.URL, g.Version, string(g.Method))
	...
	if err != nil {
		return nil, fmt.Errorf("failed to initialize GitHub SBOM iterator: %w", err)
	}
